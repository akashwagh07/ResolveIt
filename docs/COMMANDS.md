...
| `RECORD_VERIFICATION` | `complaint_id`, `evidence_relevant`, `location_consistent`, `visual_change_detected`, `confidence`, `recommendation`, `reasoning` | `AGENT3` | `AI_VERIFICATION` | Saves AI verification verdict on latest resolution. Moves to `ADMIN_VERIFICATION` (if `ADMIN_REVIEW` or low-confidence reject) or `IN_PROGRESS` (if auto-reject or `NEEDS_MORE_EVIDENCE`). |
